...
const Answer = ({ id, questionId, author, content }) => {
  const isLogged = useSelector((state) => state.user.isLogin);
  const dispatch = useDispatch();

  const handleDelete = () => {
    let confirm = window.confirm("답변을 삭제하시겠습니까?");
    if (confirm === true) {
      dispatch(deleteAnswer({ questionId: questionId, answerId: id }));
      alert("답변이 삭제되었습니다.");
        ...
        <div className="postcell">
          ...
          <UserAndOptions>
            <div className="options">
              ...
              <Link to={`answers/${id}/arevise`}>Edit</Link>
